...
(function() {
    ...
    var is = {
        ...
        set: function(shape) {
            ...
            cache.isLine = cache.isArrow = cache.isArc = cache.isDragLastPath = cache.isDragAllPaths = cache.isPencil = cache.isMarker = cache.isEraser = cache.isText = cache.isImage = cache.isPdf = false;
            cache['is' + shape] = true;
        }
    };
...
function canvasresize(id){
    ...
    addEvent(canvas, isTouch ? 'touchend touchcancel mouseup' : 'mouseup', function(e) {
        ...
        syncPoints(is.isDragAllPaths || is.isDragLastPath ? true : false, "test");
    ...
    window.addEventListener('message', function(event) {
        ...
        if(window.parent.connection.extra.roomOwner){
            ...
            console.log("RECIEVE",userid,command);
