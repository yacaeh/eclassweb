class ScreenRecorder extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      recording: false,
      screenRecorder: new screenRecorderModule()
    }
  ...
  render() {
    ...
    className={"top_icon " + (this.state.recording ? 'on' : '')} 
    id="top_record_video" 
    data-des={GetLang('TOP_RECORD_VIDEO')} 
    onClick={this.clickHandler} />
  }

  clickHandler = async (e) => {
    if (!this.state.recording) {
      await this.state.screenRecorder._startCapturing();
      this.setState({ recording: !this.state.recording });
    }
    else {
      this.state.screenRecorder._stopCapturing();
      this.setState({ recording: !this.state.recording });
    }
  }
}
